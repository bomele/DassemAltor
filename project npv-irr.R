...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Net Present Value and Internal Rate of Return Calculator"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      radioButtons(inputId = "Type_of_Calculation",
                   label = "Click Net Present Value or Internal Rate of Return",
...
server <- function(input, output,session) {
  ...
  observeEvent(input$btnCOMPUTE,{
    if(input$Type_of_Calculation==1){
      ...
      output$Result<- renderText({IRR(cf0=input$cashflow0,cf=vector_cfs,times=vector_times)})
